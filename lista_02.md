...
Classe Circulo herda de FormaGeometrica:
    ...
    Método CalcularArea():
        Retorna 3.14 * raio * raio  # Usando 3.14 como aproximação de π
```
______
...
Definir aceleracao
Definir distanciaTotal
Definir velocidadeMaxima
Definir tempoMaximo

Definir velocidadeAtual = velocidadeInicial
Definir distanciaPercorrida = 0
Definir tempo = 0
...
Enquanto distanciaPercorrida < distanciaTotal E tempo < tempoMaximo:
    ...
    distanciaPercorrida += velocidadeAtual / 60  # Dividindo por 60 para transformar velocidade por hora para por minuto
    tempo += 1
...
Função MultiplicaMatrizes(matrizA, matrizB):
    ...
    Se número de colunas em matrizA ≠ número de linhas em matrizB então:
        Retornar "Não dá para multiplicar essas matrizes."

    # Prepara a matriz nova para os resultados
    matrizNova <- novaMatriz(com número de linhas de matrizA, número de colunas de matrizB)

    # Vai calcular os valores para a matriz nova
    Para cada linha de matrizA faça:
        Para cada coluna de matrizB faça:
            soma <- 0
            Para cada elemento da linha de matrizA e coluna de matrizB faça:
                soma = soma + (elemento da linha de matrizA * elemento da coluna de matrizB)
            # Coloca a soma calculada na matriz nova
            matrizNova[linha][coluna] <- soma
...
matrizResultado <- MultiplicaMatrizes(matrizA, matrizB)
